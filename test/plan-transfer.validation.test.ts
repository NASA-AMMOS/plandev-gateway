  ...
  describe('valid', () => {
    ...
    test('optional id, model_id and tags', () => {
      expectValid({
        ...
        id: 42,
        model_id: null,
        tags: [{ tag: { name: 'imaging' } }, { tag: { name: 'external', color: '#2a9d8f' } }],
      });
    });
    ...
    test('a plan with a model and results', () => {
      expectValid(withResults({ profiles: {}, spans: [] }));
    });

    test('an activity type declaring only a name', () => {
      expectValid(withActivityType({ name: 'LegacyCalibrate' }));
    });
    ...
    test('results inheriting the plan window', () => {
      expectValid(withResults({ profiles: {}, spans: [] }));
    });
